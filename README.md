...
基于 2026 年 5 月三次野外调查的水质与浮游生物数据的定量生态学分析。
...
- **采样点**：黎照湖、香雪海、梦川、后山水池、菜根谭（5个）
- **调查批次**：2026-05-16、2026-05-23、2026-05-31（3次）
- **总样本数**：15个（5样点 × 3次）
- **记录物种**：~50种，分属13个功能类群
...
### 群落特征
- 绿藻门为优势功能群（**35.8%**）；尾滴虫和中心纲硅藻为多度最高的单一物种
- 梦川物种最丰富（39种），梦川个体最多（402）
- NMDS Stress = 0.081，群落结构拟合良好

### 群落驱动力
- **时间距离**是群落差异的最强解释因素（Mantel r = 0.326，p = 0.0004）
- 空间异质性效应不显著（p = 0.563）
- TDS 和电导率对群落有次要但显著的影响（p < 0.05）

### 关键物种
- **尾滴虫**驱动时间维度的群落演替（SIMPER 贡献率 12.6-19.6%，各时间对比中均居首位）
- **剑水蚤**驱动后山水池的空间异质性（后山水池与其他样点 SIMPER 中贡献 5.6-8.8%）
- **月牙藻**是梦川的完美指示种（IndVal = 1.000），**衣藻**是菜根谭的特征指示种（IndVal = 0.875）
...
| 原始表述 | 映射值 | 依据 |
|----------|--------|------|
| 若干 | 25~30（随机） | 修正：初期沟通失误低估，实际每个视野约25-30个体 |
| 很多 | 8 | 视野内大量（难以逐个数） |
| 大量 | 15 | 视野内密集（含"若干（大量）"） |
