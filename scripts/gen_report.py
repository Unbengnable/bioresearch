...
doc.add_paragraph(
    ...
    f'于2025年5月开展的三次野外调查数据，对浮游生物群落进行了系统的定量生态学分析。'
...
h2('1.2 采样设计')
doc.add_paragraph(
    '五个固定采样点分布在某水域不同生境类型。每个采样点分别于2025年5月16日、5月23日和5月31日'
...
for title_text, desc in [
    ('日期标准化', '原始记录中"第一次5.16"等表述统一为"2025-5.16"标准格式。'),
    ('物种-数量拆分', '原始数据以紧凑格式"物种名+数量"记录，按正则表达式拆分为独立字段。'),
    ('模糊计数规范化', '野外镜检中出现的"若干""很多""大量"等模糊表述，分别映射为数值3、8、15。'),
...
doc.add_paragraph(
    '聚类树状图显示样本大致可分为两大类群：第一大群主要为前两次调查样本，'
    '第二大群主要为第三次调查样本。这表明时间维度是影响群落组成的主要因素。'
    '后山水池的样本在聚类中位置特殊，与其物种组成的独特性有关。'
)
...
h2('8.4 方法学考量与建议')
doc.add_paragraph(
    '（1）模糊计数的不确定性：本文对"若干""很多""大量"的数值映射（3/8/15）参考了文献惯例，'
    '但不同映射方案可能影响定量结论。建议进行敏感性分析（如将"若干"映射至2~5区间），'
    '验证关键结论的稳健性。\n'
...
h2('9.2 模糊计数映射规则')
doc.add_paragraph(
    '若干 → 3（视野内少量出现，约1~5个个体）\n'
    '很多 → 8（视野内大量出现，难以逐一计数）\n'
    '大量 → 15（视野内密集分布，含"若干（大量）"变体，占据优势）\n\n'
    '注：此映射为近似处理，不可避免引入误差。建议在引用时明确说明此限制。'
)
